...
#                               RTM / TAFE-Net training path; see its docstring.
#   * PackSegInputsWithExtra -> the RTM / TAFE-Net packer (registered as
#                               type='PackSegInputsWithExtra' in the RTM configs).
#
...
# used by the RTM configs; the paper's frequency cues are packed by
# PackSegInputsWithExtra below. Kept here only for reference / non-RTM pipelines.
...
class PackSegInputsWithExtra(BaseTransform):
    ...
    RTM / TAFE-Net role:
        ...
        ``extra`` dict of frequency cues. For the RTM configs ``extra`` carries
