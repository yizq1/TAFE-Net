...
class ConditionalResizeShortestEdge(BaseTransform):
    """
    如果图片有一边小于指定尺寸，则按比例resize使最短边等于指定尺寸。
    如果两边都大于等于指定尺寸，则不进行resize。
    """
    
    def __init__(self, min_size=512, backend='cv2', interpolation='bilinear'):
    ...
    def transform(self, results):
        ...
        if min(h, w) < self.min_size:
            # 计算缩放比例，使最短边等于min_size
            scale_factor = self.min_size / min(h, w)
            ...
            if 'gt_seg_map' in results:
                gt_seg_map = mmcv.imresize(
                    ...
                    (new_w, new_h),
                    backend=self.backend,
                    interpolation='nearest'  # 分割图用最近邻插值
                )
...
@TRANSFORMS.register_module()
class get_image1(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        filename1 = results['img_path'].split('/')[-1]
        filename=os.path.join(self.data_root,filename1)
        try:
...
@TRANSFORMS.register_module()
class RandomFlip_withextra(MMCV_RandomFlip):
