...
# by the ConvNeXt RTM config (tafenet_convnext_rtm.py). It IS
...
# V2-base stage dims).
#
# Only `AsymCMNeXt_0524_convback` and its forward() are on the RTM training path;
...
class AsymCMNeXt_0524_convback(BaseModule):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stage_extra):
            ...
            if i==0:
                x_cam = self.main_branch.stem(x_cam)
            ...
            x_cam=self.main_branch.stages[i](x_cam)
