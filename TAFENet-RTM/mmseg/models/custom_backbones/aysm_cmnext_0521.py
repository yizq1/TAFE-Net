...
#     @MODELS.register_module() and instantiated by the RTM backbone
...
#     PAPER DIFF: in the RTM backbone (AsymCMNeXt_0521 / AsymCMNeXt_0524) this
...
# the RTM training path, and are left un-annotated.
# =============================================================================


# INACTIVE on the RTM path (not reached from HubVisionTransformer0521): SegFormer/MiT
...
# INACTIVE on the RTM path: mixed 3x3 / 5x5 depth-wise conv (split-channel); used only by
...
# INACTIVE on the RTM path: SegFormer-style Mix-CFN feed-forward (fc -> MixConv2d -> fc);
...
# INACTIVE on the RTM path: SegFormer Mix-FFN (fc -> DWConv -> fc); used only by the
...
# INACTIVE on the RTM path: a local copy of SegFormer's Mix-FFN; never instantiated here
...
# INACTIVE on the RTM path: SegFormer/MiT-style efficient (linear) self-attention block
...
# INACTIVE (token-select disabled): per-modal score predictor for tokenselect; in the RTM
...
class CrossAttentionModalSelector(nn.Module):
    ...
    def forward(self, x_list):
        ...
        for i, feat in enumerate(feats):
            gate_i = F.relu(self.gates[i])  # [标量], 强制为正
...
class ModalSelectorV3(nn.Module):
    ...
    def forward(self, x_list):
        ...
        for i in range(self.num_modals):
            ...
            gate_val = F.relu(self.modal_gates[i])  # 保证非负
            simi = simi * gate_val
            ...
            fused = fused + res                 # 简单残差相加

            out_list.append(fused)  # 每个模态 => [B, 1, H', W']
...
# INACTIVE on the RTM path: applies one shared module to a list of per-modal tensors; used
...
# INACTIVE on the RTM path: channel-first LayerNorm; used only by LayerNormParallel and the
...
# INACTIVE on the RTM path: per-modal channel-first LayerNorm; used only by the dead
# PatchEmbedParallel.
...
# INACTIVE on the RTM path: per-modal overlap patch-embed; used only by the unregistered
...
# INACTIVE on the RTM path: per-modal wrapper around mmseg PatchEmbed; referenced only inside a
...
# INACTIVE on the RTM path: per-modal MobileNetV3 InvertedResidual stack. Built in
# HubVisionTransformer0521.__init__ only when modals_proj=True, but the RTM config leaves
...
# `out_indices` select which stage feature maps are emitted as F_h*. The RTM
# 'tiny' arch is overridden to channels [64,128,320,512] (see arch_settings).
# PAPER DIFF: the RTM backbone AsymCMNeXt_0521 does NOT instantiate this class;
...
# and fuse the modalities (see `tokenselect`) -- but on the RTM path this modal
...
# RTM backbone AsymCMNeXt_0524[_convback]; the legacy AsymCMNeXt_0521 below also
# builds it. NOTE: that RTM parent drives this module's per-stage `.layers[i]`
