...
# -----------------------------------------------------------------------------
# This file collects the frequency "preprocessor_sec" modules used by the RTM
# training flow. The ONE module here that is on the active RTM training path is
...
# are separate DCT encoders / helpers and are NOT the FPH the RTM configs wire
...
class Doctamperdct(nn.Module):
    ...
    FAPA_v2) is commented out (inactive variant, not used by the RTM configs).
    """
    ...
    def __init__(self):
        ...
        # PAPER DIFF: optional FAPA post-processing on F_d (inactive variant, not used by the RTM configs).
...
class CATNetDCT(nn.Module):
    # Alternative DCT encoder (CAT-Net style), not the FPH the RTM configs wire in.
...
class DCTProcessor(nn.Module):
    # Alternative DCT encoder (transformer-based), not the FPH the RTM configs wire in.
    def __init__(self,
                 in_channels,
                 embed_dims=64,
                 out_channels=128,
