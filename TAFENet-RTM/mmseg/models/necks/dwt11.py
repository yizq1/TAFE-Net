...
# SegFormer/MiT-b2 baseline). The RTM configs register and instantiate the
...
# used by the RTM configs. So "DCT" in the class names is legacy naming; read
...
class HighDctFrequencyExtractor(nn.Module):
    # (dead/unused) inactive DCT high-pass variant; not on the RTM training path.
    """
    提取高频部分的DCT变换模块
...
# (dead/unused) inactive DCT low-pass variant; not on the RTM training path.
...
# below; the active neck uses DB4Wavelet, so none of them run on the RTM path.
...
def idct_2d_static(x_dct_coeffs, idct_matrix_h_t, idct_matrix_w): # 注意参数是转置后的H矩阵和原始W矩阵
...
class DCT(nn.Module):
    ...
    def __init__(self, split_ratio=0.5, normalize_outputs=True):
        ...
        self._idct_matrix_h_t_cache = None # 缓存转置后的版本
        self._idct_matrix_w_cache = None   # 缓存原始版本 (因为IDCT公式不同)
    ...
    def forward(self, x):
        ...
        # 2. 创建四个象限的掩码
        h_split = max(1, int(self.split_ratio * H_img)) # 确保至少为1
        w_split = max(1, int(self.split_ratio * W_img)) # 确保至少为1
...
# (dead/unused) inactive optimized-DCT variant; not on the RTM training path.
# (dead/unused) FFT-based DCT-like split variant; never instantiated on the RTM path (neck uses DB4Wavelet).
...
class DB4Wavelet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_flat = x.reshape(-1, 1, H, W)  # 修复：使用reshape

        # 应用小波变换  (stride-2 => each sub-band is downsampled x2)
        ll = F.conv2d(x_flat, self.kernel_ll, stride=2, padding=1)
        lh = F.conv2d(x_flat, self.kernel_lh, stride=2, padding=1)
        hl = F.conv2d(x_flat, self.kernel_hl, stride=2, padding=1)
        hh = F.conv2d(x_flat, self.kernel_hh, stride=2, padding=1)
...
class DWTNeck(BaseModule):
    ...
    `DCT(...)` alternatives are inactive variants, not used by the RTM configs.
    """
    def __init__(self,
                 in_channels,
                 out_channels):
        ...
        # lines below (Haar DWT / true DCT) are inactive variants, unused by RTM.
        # self.DWT = DWT()
        self.DWT=DB4Wavelet()
        # self.DWT=DCT()
        ...
        self.high_freq_fusion_conv = BasicConv2d(out_channels * 3, out_channels*4, kernel_size=1) # Fuse LH, HL, HH
    ...
    def forward(self, inputs):
        ...
        f1 = self.T1(f1_in) # Highest resolution  (paper F_1)
        f2 = self.T2(f2_in)                       # paper F_2
        f3 = self.T3(f3_in)                       # paper F_3
        f4 = self.T4(f4_in) # Lowest resolution   (paper F_4)
        ...
        LL_agg_down = self.LL_down(LL_agg)  # F_LL: aggregated LL brought down to F_4 res
        ...
        f4_LL = torch.cat([LL_agg_down, f4], dim=1)   # Cat(F_LL, F_4)
        f4_LL = self.one_conv_f4_ll(f4_LL)            # = F'_4

        # 4. High-resolution path -> paper F_H then F'_1 = Conv1x1(Cat(F_H, F_1)).
        # Fuse LH_agg, HL_agg, HH_agg. They are all at f1's resolution.
        fused_high_freq = torch.cat([LH_agg, HL_agg, HH_agg], dim=1)          # Cat(F_LH,F_HL,F_HH), 3C
        fused_high_freq_processed = self.high_freq_fusion_conv(fused_high_freq) # 3C -> 4C  (= F_H, pre-upsample)
        ...
        f1_fused_high = torch.cat([fused_high_freq_processed, f1], dim=1)   # Cat(F_H, F_1)

        f1_fused_high = self.one_conv_f1_fused_high(f1_fused_high)          # = F'_1

        # Return the refined pyramid (F'_1, F_2, F_3, F'_4) for the seg head.
...
class DWTFPN_dct_v6(BaseModule):
    """RTM-registered neck: DFDE (DWTNeck) followed by a standard mmseg FPN.

    This is the class the RTM configs instantiate. It equals the paper's DFDE
...
class DWT(nn.Module):
    ...
    def forward(self, x):
        ...
        # 如果高度或宽度是奇数，进行填充
        if h % 2 != 0:
            x = F.pad(x, (0, 0, 0, 1))  # 在高度方向填充一行
        if w % 2 != 0:
            x = F.pad(x, (0, 1, 0, 0))  # 在宽度方向填充一列
...
class DCT_ARB(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.rb(x)          # direction-specific residual block
        y = self.attention(y)   # channel + spatial attention (AM)
...
class DCT_AGF(nn.Module):
    ...
    def __init__(self, channels, subband_type):
        ...
        self.ARB = DCT_ARB(channels, subband_type)   # builds the ARB attention/guidance map
        self.GF = GF(r=2, eps=1e-2)                   # guided filter that fuses across scales
        self.subband_type = subband_type
...
class DCT_AGFG(nn.Module):
    ...
    def forward(self, f1, f2, f3, f4):
        ...
        y1 = self.GF1(f2, f1)   # guide f1 with next-finer sub-band f2
        y2 = self.GF2(f3, y1)   # guide the result with f3
        y3 = self.GF3(f4, y2)   # guide the result with the finest sub-band f4
