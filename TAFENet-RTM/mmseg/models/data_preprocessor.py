...
# right before it calls the segmentor. The RTM configs use
...
# no extras support; it is NOT on the RTM/TAFE-Net training path in this repo.
# =============================================================================
...
class SegDataPreProcessorWithExtra(BaseDataPreprocessor):
    ...
    tensors that the RTM dataset pipeline attaches under ``data['extra']``.
    ...
    def forward(self, data: dict, training: bool = False) -> Dict[str, Any]:
        ...
        inputs = data['inputs']
        data_samples = data.get('data_samples', None)
        # `extras` is the auxiliary bundle produced by the RTM pipeline: a
