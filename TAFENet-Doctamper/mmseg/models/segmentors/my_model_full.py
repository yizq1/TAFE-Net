...
# in the DocTamper configs tafenet_segformer_doctamper.py / tafenet_convnext_doctamper.py). It is the entry
...
# Aggregation", AAAI 2026). Two DocTamper training variants share this class:
#   * SegFormer variant  (config tafenet_segformer_doctamper.py)          = paper "TAFE-Net"
#       backbone AsymCMNeXt_0524,          main visual encoder = SegFormer/MiT-b2,
#       decode_head = SegformerHead (= the paper's SegFormerHead).
#   * ConvNeXt  variant  (config tafenet_convnext_doctamper.py) = paper "TAFE-Net*"
#       backbone AsymCMNeXt_0524_convback, main visual encoder = ConvNeXt V2-base,
#       decode_head = FPNHead.
# Both train ONLY on DocTamper (via _base_ datasets/rtm_crop_doctamper.py).
#
...
#             WithExtra). For the DocTamper configs it holds:
...
#
# NOTE: backbone_sec / fuser / preprocessor / extra_head are all UNUSED by the DocTamper
#       configs (no backbone_sec is set), so those branches below are inactive.
# =============================================================================
@MODELS.register_module()
class MyModelFull(BaseSegmentor):
    """Top-level tamper-localization segmentor for the DocTamper training configs.
    ...
    def forward_encoder(self, inputs, extras):
        ...
        # --- (0) optional RGB pre-processing. Unused in the DocTamper configs (no
        #     `preprocessor` set), so `inputs` stays the raw normalized RGB image. ---
        if self.with_preprocessor:
            inputs = self.preprocessor(inputs)

        # --- select the source for the secondary stream. key_sec is None in the DocTamper
        ...
        # list. For the DocTamper configs it is, in order (self.extra_names accordingly):
        #   [0] 'img' -> low-freq extractor (SegFormer: MultiScaleMultiLowFreqExtractor / ConvNeXt: LowDctFrequencyExtractor)   : RGB -> low-freq DCT view = paper I_lf  (3ch)
        #   [1] 'img' -> high-freq extractor (SegFormer: MultiScaleHighFrequencyExtractor / ConvNeXt: HighDctFrequencyExtractor)  : RGB -> high-freq DCT view = paper I_hf (3ch)
        ...
        if self.with_preprocessor_sec:
            if isinstance(self.preprocessor_sec, nn.ModuleList):
                ...
                # single-extractor case (not used by the DocTamper configs)
        ...
        # merge_input=True in the DocTamper configs: hand the backbone a 2-element list
        ...
        if self.with_backbone_sec:
            ...
            # NOT used by the DocTamper configs (backbone_sec is unset), kept for other cfgs.
    ...
    def _decode_head_forward_train(self, inputs: List[Tensor], extras,
        ...
        use_extra is False in the DocTamper configs, so the plain head.loss() path is taken.
        """
        losses = dict()
        if self.use_extra:
