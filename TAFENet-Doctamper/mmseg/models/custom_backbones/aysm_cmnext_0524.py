...
#
# The ACTIVE class on the DocTamper training path in this file is `AsymCMNeXt_0524`
...
class ConvNeXt_0524(BaseModule):
    ...
    TAFE-Net role: MFFE high-frequency CNN branch = CNNBlocks (eq6 -> F_h). NOTE/DEVIATION:
    in THIS file AsymCMNeXt_0524 builds its high-freq branch via timm 'convnextv2_tiny', so
    this registered class is the config-selectable alternative, not the default run-path here.
...
# in this file but is unused by the DocTamper configs). Both branches keep the input
# channel count, so the sum has the same NCHW shape as the input.
class SCSEModule_improve(nn.Module):
    """Paper scSE (eq(8)) fusion, improved variant: channel attention CCALayer + spatial attention ESA, summed.

    Used inside FU/FU1/FU2/FU3 to fuse Cat(F_h, F_l) -> F_hl (and F_hl2 + FPH F_d -> F_cf2)."""
    def __init__(self, in_channels, reduction=16):
        super().__init__()
...
class AsymCMNeXt_0524(BaseModule):
    ...
    def __init__(self,
        ...
        """Build the MFFE multi-branch encoding backbone and assemble the per-stage fusion units.

        Three/four branches (paper MFFE eq(5)-(8), VFIM input eq(1)-(4)):
          - main_branch  : backbone_main, SegFormer visual backbone (RGB Iv), provides backbone feature x_cam.
          - extra_branch : backbone_extra, low-freq Transformer branch = TransformerBlocks (eq7, F_l); input preprocessed by prim1 = VFIM (eq2/eq4).
          - extra1_branch: ConvNeXtV2 high-freq CNN branch = CNNBlocks (eq6, F_h, DCT input); input preprocessed by prim2 = VFIM (eq1/eq3).
          - x_extra_3    : third extra frequency-domain feature = FPH DCT F_d (eq5), fused in stage1.
        conv11 aligns the ConvNeXt per-stage channels [96,192,384,768] to [64,128,320,512];
        FU/FU1/FU2/FU3 are the per-stage scSE fusion units (eq8, scSE attention + convolution), injected back into the backbone in a residual manner.
        """
        super().__init__(init_cfg=init_cfg)
        ...
        # extra1_branch = high-freq CNN branch = ConvNeXt-V2 CNNBlocks (eq6 -> F_h)
        # DEVIATION: built from timm 'convnextv2_tiny' rather than the local ConvNeXt_0524 class
        ...
        # SCSEModule class also exists in this file but is unused by the DocTamper configs.
        #
        ...
        # self.FU1 = nn.ModuleList([nn.Sequential(SCSEModule(256), nn.Conv2d(256, 128, 3, 1, 1), nn.BatchNorm2d(128), nn.ReLU(True))])  # (inactive variant, not used by the DocTamper configs)
        ...
        # self.FU1 = nn.ModuleList([nn.Sequential(SCSEModule(160), nn.Conv2d(160, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU(True))])  # (inactive variant, not used by the DocTamper configs)
        # self.FU1.append(nn.Conv2d(64, 64, 1, 1, 0))
        ...
        # FU3 is never used on the forward path (all self.FU3 calls are commented out),
        # so it is disabled here to avoid registering unused parameters.
    ...
    def compute_feature_distance(self, x_f, x_f_1):
        ...
        # 计算当前通道的余弦相似度
        cosine_similarity = F.cosine_similarity(x_f, x_f_1, dim=1, eps=1e-6)  # output shape is (N, H, W)

            # 转换为余弦距离
        cosine_distance = 1 - cosine_similarity

            # 恢复成形状 (N, 1, H, W)，以便后续拼接
        channel_anomaly_map = cosine_distance.unsqueeze(1)  # output shape is (N, 1, H, W)
    ...
    def forward(self, x):
        ...
        if self.in_stages is not None:
            if max(self.in_stages) > 0:
                # (inactive branch for the DocTamper configs, which use in_stages=(0,0)):
        ...
        for i in range(self.num_stage_extra):
            # import pdb;pdb.set_trace()
            # layer = SegFormer visual stage; layer_extra = low-freq Transformer stage (eq7)
            ...
            if self.in_stages is not None:
                if i == in_stage:
                    ...
                    # eq(7): low-freq Transformer branch — overlap patch-embed of the Il_hat stream
                    x_extra_1,_= layer_extra[0](x_extra_1)
            ...
            # modified on 2025-06-21
            # x_f=x_extra_1[0]
            # x_f = x_f.flatten(2).transpose(1, 2)
            # eq(7): Transformer blocks + norm on the low-freq stream -> F_l (x_f)
            x_f=x_extra_1
            ...
            # import pdb;pdb.set_trace()
            # conv11: align ConvNeXt-V2 channels to SegFormer stage dims -> F_h
            ...
            if i==0:
                ...
                # July 4th: this is what was used before the ablation
                # eq(8): FU1[0] = scSE(Cat(F_h1,F_l1)) -> F_hl1; then residual add into SegFormer stage0 -> F1
                ...
                # x_fused = self.FU3[1](ext_2)+x_cam
                # x_cam=x_fused
                ...
                # x_fused = self.FU4[1](ext_2)+x_cam
                ...
                # x_fused=x_extra_3+x_cam
                # ext1 = self.FU2[0](torch.cat((x_f, x_f_1), dim=1))
                # # ext1=x_f+x_f_1
                # ext1_1=self.FU2[1](ext1)
                # ext_fph = self.FU[0](torch.cat((x_extra_3, x_cam), dim=1))
                # x_fused = self.FU[1](ext_fph)+x_cam+ext1_1
                # x_cam=x_fused
...
'''
    def loss(self, x,data_samples):
        ...
        for i in range(self.num_stage_extra):
            ...
            # in ascformer_rtm_img_img1_true_cosine_frfm's 20250319_145114
            # x, x_f = self.FRMs[i](x_cam, x_f_end)
            # x_fused = self.FFMs[i](x_cam, x_f_end)
