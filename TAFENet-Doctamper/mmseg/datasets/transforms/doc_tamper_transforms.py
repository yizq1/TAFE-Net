"""Data transforms for document tampering detection.

Mainly performs JPEG re-compression augmentation and uses jpegio to parse the
DCT coefficients and quantization table of the compressed image, for the
model's DCT/frequency branch.
"""
import os
import pickle
import random
...
# runs inside the DocTamper (RealTextManipulation) training pipeline, right before
...
class RandomJpegCompressAndLoadInfo(BaseTransform):
    ...
    Role in the TAFE-Net / DocTamper training flow
    ----------------------------------------
